...
class Grid:
    ...
    def interact(self, photon_list: PhotonList, absorb, iabsorb, interact, iphotons, scattering=False, learning=False):
        ...
        if not scattering and nabsorb > 0:
            wp.launch(kernel=self.update_frequency,
                      dim=(nabsorb,),
                      inputs=[photon_list, new_frequency, self.dust.ml_kabs(wp.to_torch(photon_list.p)[iabsorb], wp.to_torch(photon_list.amax)[iabsorb], wp.to_torch(new_frequency)), 
                              self.dust.ml_ksca(wp.to_torch(photon_list.p)[iabsorb], wp.to_torch(photon_list.amax)[iabsorb], wp.to_torch(new_frequency)), iabsorb])
        t2 = time.time()
    ...
    def ml_step(self, photon_list, s, iphotons):
        ...
        wp.launch(kernel=self.update_frequency,
                  dim=(nphotons,),
                  inputs=[photon_list, frequency, self.dust.ml_kabs(wp.to_torch(photon_list.p)[iphotons], wp.to_torch(photon_list.amax)[iphotons], frequency), self.dust.ml_ksca(wp.to_torch(photon_list)[iphotons], wp.to_torch(photon_list.amax)[iphotons], frequency), iphotons])
    ...
    def propagate_photons(self, photon_list: PhotonList, use_ml_step=False, learning=False, debug=False, timing={}):
        with wp.ScopedDevice(self.device):
            ...
            t1 = time.time()
            photon_list.kabs = wp.from_torch(self.dust.ml_kabs(wp.to_torch(photon_list.p), wp.to_torch(photon_list.amax), wp.to_torch(photon_list.frequency), iphotons))
            photon_list.ksca = wp.from_torch(self.dust.ml_ksca(wp.to_torch(photon_list.p), wp.to_torch(photon_list.amax), wp.to_torch(photon_list.frequency), iphotons))
    ...
    def propagate_rays(self, ray_list: PhotonList, frequency, pixel_size):
        with wp.ScopedDevice(self.device):
            ...
            ray_list.kext = wp.from_torch(torch.cat([self.dust.ml_kext(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), torch.tensor(frequency[i], dtype=torch.float32).expand(nrays), \
                    iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
            ray_list.ray_albedo = wp.from_torch(torch.cat([self.dust.ml_albedo(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), torch.tensor(frequency[i], dtype=torch.float32).expand(nrays), \
                    iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
            ...
            while nrays > 0:
                ...
                if nrays > 0:
                    ...
                    ray_list.kext = wp.from_torch(torch.cat([self.dust.ml_kext(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), torch.tensor(frequency[i], dtype=torch.float32).expand(nrays), \
                            iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
                    ray_list.ray_albedo = wp.from_torch(torch.cat([self.dust.ml_albedo(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), \
                            torch.tensor(frequency[i], dtype=torch.float32).expand(nrays), iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
        ...
        with wp.ScopedDevice(self.device):
            ...
            while nrays > 0:
                ...
                if nrays > 0:
                    ...
                    ray_list.kext = wp.from_torch(torch.cat([self.dust.ml_kext(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), torch.tensor(frequency[i], dtype=torch.float32).expand(iray_original.size(0)), \
                            iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
                    ray_list.ray_albedo = wp.from_torch(torch.cat([self.dust.ml_albedo(wp.to_torch(ray_list.p), wp.to_torch(ray_list.amax), \
                            torch.tensor(frequency[i], dtype=torch.float32).expand(iray_original.size(0)), iray).unsqueeze(1) for i in range(frequency.size)], axis=1))
...
class UniformCartesianGrid(Grid):
    def __init__(self, ncells=9, dx=1.0*u.au, device="cpu"):
        ...
        Parameters
        ----------
        ncells : int or tuple of int
            The number of cells in each dimension. If an int is provided, the same number of
        ...
        dx : astropy Quantity or tuple of astropy Quantity
            The size of each cell in each dimension. If a Quantity is provided, the
            same cell size will be used in all dimensions. If a tuple is provided, it should
            contain three Quantities specifying the cell size in the x, y, and z
            dimensions.
        device : str
...
class LogUniformSphericalGrid(UniformSphericalGrid):
    def __init__(self, ncells=9, rmin=0.1*u.au, rmax=4.5*u.au, mirror=True, device="cpu"):
        ...
        ncells : int or tuple
            ...
            cells will be used in all dimensions. If a tuple is provided, it should contain
            three integers specifying the number of cells in the x, y, and z dimensions.
        rmin : astropy Quantity
            The minimum radius of the grid.
        rmax : astropy Quantity
        ...
        self.distance_unit = rmax.unit

        _w1 = np.hstack(([0.], np.logspace(np.log10(rmin.value), np.log10(rmax.value), n1)))
        if mirror:
            _w2_max = np.pi / 2
        else:
            _w2_max = np.pi
    ...
    def emit(self, nphotons, wavelength="random", scattering=False, learning=False, timing={}):
        ...
        with wp.ScopedDevice(self.device):
            photon_list.radius = wp.array(np.zeros(nphotons), dtype=float)
            photon_list.logradius = wp.array(np.zeros(nphotons), dtype=float)
            photon_list.theta = wp.zeros(nphotons, dtype=float)
            photon_list.phi = wp.zeros(nphotons, dtype=float)
            photon_list.sin_theta = wp.zeros(nphotons, dtype=float)
            photon_list.cos_theta = wp.zeros(nphotons, dtype=float)
    ...
    def photon_loc(photon_list: PhotonList,
        ...
        photon_list.radius[ip] = wp.sqrt(photon_list.position[ip][0]**2. + photon_list.position[ip][1]**2. + photon_list.position[ip][2]**2.)
        photon_list.logradius[ip] = wp.log10(photon_list.radius[ip])
